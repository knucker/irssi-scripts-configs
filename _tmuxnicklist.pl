# tmuxnicklist.pl
#    Copyright (C) 2014  Knucker <www.knucker [at] gmail.com>
#
...
#
# TODO:
#		- Do not make "Irssi::command('nicklist fifo') necessary. (TMUX mode)
#
#
# Thanks to Alexander Kratzsch (nicklist_tmux.pl)
# and to Wouter Coekaerts (nicklist.pl)
...
use POSIX;
use constant DATETIME => strftime("%H%M%S%m%d%Y", localtime);
$VERSION = '0.4.7';
%IRSSI = (
    authors     => 'Knucker Blind',
    contact     => 'www.knucker [at] gmail.com' .
    name        => 'tmuxnicklist',
    description => 'This script makes it ' .
                   'possible to use the  ' .
                   'great nicklist.pl script' .
									 'with tmux.',
	url 		=> 'my',
    license     => 'GPLv2',
    changed     => '14/08/2014'
...
2. TMUX MODE
- start irssi inside tmux ("tmux irssi")
- in irssi: /NICKLIST TMUX (only the first time, to create the fifo)
- back in irssi:
    /SET nicklist_heigth <height of nicklist>
    /SET nicklist_width <width of nicklist>
    /NICKLIST TMUX
- You can use '/set NICKLIST_TMUX_AUTOSTART ON' to automatically start when loaded.
...
my ($OFF, $SCREEN, $FIFO, $TMUX) = (0,1,2,3); # modes
...
sub cmd_tmux_start {
	if (!defined($ENV{'TMUX'})) {
		Irssi::print 'tmux not detected, tmux mode only works inside tmux';
		return;
	}

	Irssi::signal_add('terminal resized', \&cmd_tmux_resized);
	...
	system 'tmux -q split-window -v "cat ' . Irssi::settings_get_str('nicklist_fifo_path') . '"';
	cmd_tmux_resized();	
...
sub tmux_stop {
	Irssi::signal_remove('terminal resized', \&cmd_tmux_resized);
	# kill the nicklist-pane
	system 'tmux -q kill-pane -t 1';
	fifo_stop();

	return 0;
}
sub cmd_tmux_resized {
	# just resize the pane
	system 'tmux -q select-pane -t 0';
	system 'tmux -q set-window-option other-pane-width ' . Irssi::settings_get_int('nicklist_width');
	system 'tmux -q select-layout main-vertical';
	system 'tmux -q select-pane -t 0';
...
Irssi::command_bind('nicklist tmux',\&cmd_tmux_start);
...
Irssi::signal_add_first('gui exit' => \&tmux_stop);
...
Irssi::settings_add_str('nicklist', 'nicklist_fifo_path', Irssi::get_irssi_dir . '/nicklistfifo' . DATETIME);
...
if (uc(Irssi::settings_get_str('nicklist_automode')) eq 'SCREEN') {
	...
	cmd_fifo_start();
} elsif (uc(Irssi::settings_get_str('nicklist_automode')) eq 'TMUX') {
	cmd_tmux_start();
}
